# Мои уточнения по ТЗ
Файл docker-compose.yaml - содержит задание 1 + 3 запускется на localhost:7575 (конфигфайл для nginx находится nginx/nginx.conf)
Файл docker-compose2.yaml - это первое задание запускается на localhost:8000 (Оставил его, чтобы первое задание было отдельно еще)

Файл workflow для CI находится в папке - .github/workflows/check.yml (Не уверен что в нем нужно было делать 2 сборки контенеров, но сначала была выполнена первая часть задания, а потом уже добавил вторую)

P.S.
Остались конечно вопросы с CI так как впревые работал с GithubActions и возможно что-то не верно понял по заданию
Но в целом задание интересное много чего узнал нового :)
